...
n_z = int(rz_max/angle_a)+1
n_h = int(rh_max/angle_a)+1
...
out = np.zeros((len(R_bins)-1,n_z*n_h))
i = 0
while (r_z < rz_max) and (i < n_h*n_z):
    while r_h < rh_max:
        ...
        out[:,i] = np.histogram(R[boo_ar],bins=R_bins)[0]
...
# number of lasers shot in each voxel path
B = int((angle_a/l_angle)**2)
